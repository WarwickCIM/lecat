...
function(input, output, session) {
  ...
  data <- reactiveValues(
    ...
    lecat_lexicon = data.frame(Type = 'No lexicon loaded yet',
                               stringsAsFactors = FALSE),

    # Reactive value: corpus ----
    lecat_corpus = data.frame(Type = 'No corpus loaded yet',
                              stringsAsFactors = FALSE),

    # Reactive value: lookup table ----
    lecat_lookup_table = data.frame(Type = 'No lookup table loaded yet',
                                    stringsAsFactors = FALSE),

    # Reactive value: raw results ----
    lecat_raw_result = data.frame(Type = 'lecat not run yet',
                                  stringsAsFactors = FALSE),

    # Reactive value: diagnostics ----
    lecat_diagnostics = data.frame(Type = 'No diagnostics created yet',
                                   stringsAsFactors = FALSE),

    # Reactive value: cotable ----
    lecat_cotable = data.frame(Type = 'Cooccurrence table not generated yet',
                               stringsAsFactors = FALSE),
  ...
  example_corpus <- data.frame(
    ...
    description = c(
      'In this iphone and ipad delivered lecture James talks about a new tool.',
  ...
  output$lecat_lexicon <-
    DT::renderDataTable(data$lecat_lexicon,
                        options = list(pageLength = 5, scrollX = TRUE))
  output$lecat_lookup_table <-
    DT::renderDataTable(data$lecat_lookup_table,
                        options = list(pageLength = 5, scrollX = TRUE))
  output$lecat_diagnostics <-
    DT::renderDataTable(data$lecat_diagnostics,
                        options = list(pageLength = 5, scrollX = TRUE))

  # Flag checking if lookup table, corpus and lexicon are loaded ----
  output$lecat_flag <- reactive(if (data$corpus_loaded &
                                    data$lexicon_loaded &
                                    data$lookup_table_loaded) {
    'lecat_ready'
  } else {
    'lecat_not_ready'
  })
  ...
  output$lecat_analysis_flag <-
    reactive(if (data$lecat_analysis_complete) {
      'analysis_complete'
    })
  ...
  observeEvent(input$lecat_lexicon_file, {
    ...
    # inform the user
    shiny::showNotification('Formatting lexicon to long format',
                            type = 'message',
                            duration = 2)
  ...
  observeEvent(input$lecat_lookup_table_file, {
    ...
    tryCatch({
      ...
      if (('Type' %in% names(lookup_table)) &
          ('Column' %in% names(lookup_table))) {
        # remove accidental duplicate rows
        lookup_table <- unique(lookup_table[, c('Type', 'Column')])

        # check for missing values
        if (sum(!complete.cases(lookup_table)) > 0) {
          # if there are rows with missing Type or Column}
        ...
        shiny::showNotification('Lookup Table not loaded: Column names Type and Column required',
                                type = 'error')
      }
    },
    error = function(e) {
  ...
  observeEvent(input$lecat_run_analysis_button, {
    ...
    if (mean(lookup_table_types %in% lexicon_types) == 1) {
      ...
      tryCatch({
        ...
        x <-
          run_lecat_analysis(
            ...
            advanced_mode = input$lecat_advanced_mode
          )
      ...
      shiny::showNotification('Lookup table types do not match lexicon types. Please check these files',
                              type = 'error')
  ...
  # Extract the output type requested by the user ----
